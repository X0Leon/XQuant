...
class NaivePortfolio(Portfolio):
    ...
    def __init__(self, bars, events, start_date, initial_capital=1.0e5):
        ...
        self.all_positions = self.construct_all_postions() # 字典列表
    ...
    def update_timeindex(self, index):
    	"""
    	用于追踪新的持仓市值
    	向持仓头寸中加入新的纪录，也就是上一根完整bar（k线+成交量）
    	...
    	dp['datetime'] = bars[self.symbol_list[0]][0][1] # 问题：
    	                                                 # 此处应该bars[][0].datetime?

    	for s in self.symbol_list:
    		dp[s] = self.current_postitions[s]
    	# 添加当前头寸
    	self.all_positions.append[dp] # 注意all_postions是字典列表
    	# 更新持仓，字典
    	dh = dict((k,v) for k,v in [(s,0) for s in self.symbol_list])
    	dh['datetime'] = bars[self.symbol_list[0]][0][1] # 问题：
    	                                                 # 还是应该bars[][0].datetime
